...
def ramps_vel(v_prev, v_target, t_prev, t_now, ramp_rate):
...
def ramped_twist(prev, target, t_prev, t_now, ramps):
    tw = Twist()
    tw.angular.z = ramps_vel(prev.angular.z, target.angular.z, t_prev, t_now, ramps[0])
    tw.linear.x = ramps_vel(prev.linear.x, target.linear.x, t_prev, t_now, ramps[1])
    return tw
